            ...
            case "memory":
                {
                    ...
                    if(rowM.length == 0){
                        const[resM] = await pool.query("INSERT INTO memory_ranck (idUser, userName, puntos, date, level) VALUES (?, ?, ?, ?, ?);", [idUser,nameUser,puntuacion,date,level ])
            ...
            case "mines":
                {
                    ...
                    if(rowMi.length == 0){
                        const[resMi] = await pool.query("INSERT INTO mine_ranck (idUser, userName, puntos, date, level) VALUES (?, ?, ?, ?, ?);", [idUser,nameUser,puntuacion,date,level ])
                        return {status:200,message:"createNewRegister"}
                    }else{
                        const [datesMi] = rowMi;
            ...
            case "snake":
                {
                    ...
                    if(rowS.length == 0){
                        const[resS] = await pool.query("INSERT INTO snake_ranck (idUser, userName, puntos, date) VALUES (?, ?, ?, ?);", [idUser,nameUser,puntuacion,date])
                        return {status:200,message:"createNewRegister"}
                    }else{
                        const [datesS] = rowS;
